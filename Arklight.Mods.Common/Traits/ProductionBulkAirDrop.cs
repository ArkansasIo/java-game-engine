...
{
	[Desc("Deliver multiple units via skylift. Works with BulkProductionQueue")]
	public class ProductionBulkAirdropInfo : ProductionInfo
	{
		[NotificationReference("Speech")]
		[Desc("Speech notification to play when a unit is delivered.")]
		public readonly string ReadyNotification = "Reinforce";
	...
	sealed class ProductionBulkAirdrop : Production
	{
		RallyPoint rp;

		public ProductionBulkAirdrop(ActorInitializer init, ProductionBulkAirdropInfo info)
			: base(init, info) { }
		...
		public bool DeliverOrder(Actor producer, List<(ActorInfo Actor, int Resources, int Cash)> orderedActors, string productionType, BulkProductionQueue queue)
		{
			if (IsTraitDisabled || IsTraitPaused)
				return false;
			var info = (ProductionBulkAirdropInfo)Info;
			...
			owner.World.AddFrameEndTask(w =>
			{
				if (!producer.IsInWorld || producer.IsDead)
				{
					...
					if (anotherProducer.Trait != null && anotherProducer.Trait is ProductionBulkAirdrop anotherStarport)
				...
				var exitCell = producer.Location;
				if (exit != null)
					exitCell = producer.Location + exit.Info.ExitCell;

				var destinations = rp != null && rp.Path.Count > 0 ? rp.Path : [exitCell];

				// Aircraft fly in under their own power.
				foreach (var orderedAircraft in orderedActors.Where(actor => actor.Actor.HasTraitInfo<AircraftInfo>()))
				{
					...
					var aircraft = w.CreateActor(orderedAircraft.Actor.Name,
					[
