...
namespace Arklight.Mods.Cnc.Graphics
{
	public sealed class VoxelLoader : IDisposable
	{
		...
		ModelVertex[] GenerateSlicePlane(int su, int sv, Func<int, int, VxlElement?> first, Func<int, int, VxlElement?> second, Func<int, int, float3> coord)
		{
			...
			Arklight.Graphics.Util.FastCopyIntoChannel(s, colors, SpriteFrameType.Indexed8);
			Arklight.Graphics.Util.FastCopyIntoChannel(t, normals, SpriteFrameType.Indexed8);
